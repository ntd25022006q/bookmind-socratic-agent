...
async def questioner_node(state: ResearchState, config=None) -> dict:
    ...
    if stream_queue:
        await stream_queue.put({
            "type": "node_start",
            "node": "questioner"  # Maps to node-questioner in UI
        })
        
    print_agent_start("Questioner Agent", "Đặt câu hỏi Socratic kích thích tư duy phản biện")
    llm = create_llm(MODEL_RESEARCHER_AGENT, config=config, streaming=True)
    
    call_config = {}
    if stream_queue:
        from src.utils.llm_factory import QueueCallbackHandler
        call_config["callbacks"] = [QueueCallbackHandler(stream_queue, "questioner")]
    ...
    print_agent_complete("Questioner Agent", duration, tokens)
